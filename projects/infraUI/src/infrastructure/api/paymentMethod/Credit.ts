const pay = async (finalPrice: number, userId: string, token: string): Promise<string> => {
  try {
    await authorize(finalPrice, userId, token);
    return await capture(token)
  } catch (error) {
    throw error
  }
}

const authorize = async (amount: number, userId: string, token: string) => {
...
const capture = async (token: string): Promise<string> => {
