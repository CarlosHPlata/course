...
describe('At mapping a pokemon from the poke api', () => {
  ...
  it('should map the moves from the pokemon', async () => {
    ...
    expect(moves[ Math.floor(moves.length / 2) ]).toEqual(pokemonMock.moves[Math.floor(moves.length / 2)]);
    expect(moves[ moves.length -1 ]).toEqual(pokemonMock.moves[ moves.length -1 ]);
  });
});